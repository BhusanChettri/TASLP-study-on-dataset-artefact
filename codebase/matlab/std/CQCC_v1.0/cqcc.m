function  [CQcc, LogP_absCQT, TimeVec, FreqVec, Ures_LogP_absCQT, Ures_FreqVec] = ...
    cqcc(x, fs, B, fmax, fmin, d, cf, ZsdD, CMVN)

%   Constant Q cepstral coefficients
%   Usage:  CQcc = cqcc(x, fs, B, fmax, fmin, d, cf, ZsdD, CMVN)
%
%   Input parameters:
%         x        : input signal
...
%%% LOG-ENERGY COEFFICIENT

if strfind(ZsdD, 'E')
    logEnergy = log(mean(absCQT.^2)+eps);
    % overwrite the zero coefficient with the log-energy coefficient
    CQcepstrum (1,: ) = logEnergy;
end


%%% DYNAMIC COEFFICIENTS
%if strfind(ZsdD, 'Z') || strfind(ZsdD, 'E'); scoeff = 1; else scoeff = 2; end
%CQcepstrum_temp = CQcepstrum(scoeff:cf+1,:);

if strfind('ZsdD', 'Z') || strfind('ZsdD', 'E'); 
    scoeff = 1; 
else
    scoeff = 2;
end
...
if strcmp(strrep(ZsdD,'Z',''), 'sdD') || strcmp(strrep(ZsdD,'E',''), 'sdD')
    CQcc = [CQcepstrum_temp; Deltas(CQcepstrum_temp,f_d); ...
        Deltas(Deltas(CQcepstrum_temp,f_d),f_d)];
elseif strcmp(strrep(ZsdD,'Z',''), 'sd') || strcmp(strrep(ZsdD,'E',''), 'sd')
    CQcc = [CQcepstrum_temp; Deltas(CQcepstrum_temp,f_d)];
elseif strcmp(strrep(ZsdD,'Z',''), 'sD') || strcmp(strrep(ZsdD,'E',''), 'sD')
    CQcc = [CQcepstrum_temp; Deltas(Deltas(CQcepstrum_temp,f_d),f_d)];
elseif strcmp(strrep(ZsdD,'Z',''), 's') || strcmp(strrep(ZsdD,'E',''), 's')
    CQcc = CQcepstrum_temp;
elseif strcmp(strrep(ZsdD,'Z',''), 'd') || strcmp(strrep(ZsdD,'E',''), 'd')
    CQcc = Deltas(CQcepstrum_temp,f_d);
elseif strcmp(strrep(ZsdD,'Z',''), 'D') || strcmp(strrep(ZsdD,'E',''), 'D')
    CQcc = Deltas(Deltas(CQcepstrum_temp,f_d),f_d);
elseif strcmp(strrep(ZsdD,'Z',''), 'dD') || strcmp(strrep(ZsdD,'E',''), 'dD')
    CQcc = [Deltas(CQcepstrum_temp,f_d); Deltas(Deltas(CQcepstrum_temp,f_d),f_d)];
end

%%% CEPSTRAN MEAN AND VARIANCE NORMALIZATION
if CMVN
    CQcc = cmvn(CQcc);    
end
