...
# Thresholds of the original SVM systems trained on ivectors
trainThreshold=0.043
devThreshold=0.3972
evalThreshold=0.506
scoreFormat='single-column'
...
do
  ...
  for subset in 'train' 'dev' #'eval'  #we do not have dtmf annotations on eval
  do
   if [ $subset == 'train' ]; 
    then
   ...
   useAnnotations=$annotationBase/$subset/v2/detailed_annotations/dtmf_last_columns_removed.txt  #This is same as 'dtmf.txt' but last column removed for script compatibility
